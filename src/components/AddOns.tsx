  ...
  return (
    <div>
      {addOns.map((addOn) => (
        <div
          key={addOn.id}
          className={`mb-2 grid grid-cols-[.1fr,1fr,.3fr] items-center gap-4 rounded-md border border-lightGray p-3 ${isAddOnSelected(addOn.id) ? "border-purplishBlue bg-purplishBlue bg-opacity-10" : ""} `}
        >
          <input
            type="checkbox"
